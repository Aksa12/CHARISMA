...
RESULTS_COLS = [
    ...
    "row_id", "replicate", "slot_id", "scenario_idx",
...
def run_with_schedule(
    ...
    total_slots = total_rows * num_repeats

    # Build pending slots as (schedule_index, replicate)
    pending_slots = []
    for i in range(total_rows):
        row_id = str(schedule.loc[i, "row_id"])
        for r in range(num_repeats):
            slot_id = f"{row_id}::r{r}"
            if slot_id not in done_ids:
                pending_slots.append((i, r))

    print(f"[Runner] schedule_rows={total_rows} total_slots={total_slots} done={len(done_ids)} pending={len(pending_slots)}")

    processed = 0
    errors = 0

    for (i, replicate) in pending_slots:
        srow = schedule.loc[i]
        # Parse minimal fields
        row_id = str(srow["row_id"])
        slot_id = f"{row_id}::r{replicate}" 
        ...
        seed = int(GLOBAL_SEED + (i * num_repeats) + replicate)
        ...
        processed += 1
        pct = 100.0 * (len(done_ids) + processed) / total_slots
        print(f"[Runner] [{len(done_ids)+processed}/{total_slots}] {slot_id} -> {row_out['status']} ({pct:.1f}%)")


    print(f"[Runner] Finished. newly_processed={processed}, errors={errors}, total_done={len(done_ids)+processed}/{total_slots}")
