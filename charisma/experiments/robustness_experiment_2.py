...
def read_done_rows(results_csv: str) -> set:
    if not os.path.exists(results_csv):
        return set()
    df = pd.read_csv(results_csv, dtype=str)
    if "condition_idx" not in df.columns or "status" not in df.columns:
        return set()
    return set(df.loc[df["status"] == "ok", "condition_idx"].astype(str))
...
def lookup_scenario_row(scenario_idx: str, scenario_df: pd.DataFrame) -> pd.Series:
    print("scenario", scenario_idx)
    m = scenario_df[scenario_df["scenario_idx"] == scenario_idx]
    if len(m) == 1:
        return m.iloc[0]
    row = scenario_df.iloc[scenario_idx]
    return row
...
RESULTS_COLS = [
    ...
    "condition_idx", "scenario_idx", "subcategory", "pair_id",
...
def run_with_schedule(
    ...
    schedule = schedule.sort_values("condition_idx").reset_index(drop=True)
    chars = load_characters(characters_csv)
    scenario_df = pd.read_csv(scenario_csv)

    scenario_df["scenario_idx"] = range(1, len(scenario_df) + 1)  # assuming 1-based indexing in schedule
    
    
    # Resume: skip done condition_ids
    done_ids = read_done_rows(results_csv)
    total = len(schedule)
    pending_idxs = [i for i in range(total) if schedule.loc[i, "condition_idx"] not in done_ids]
    ...
    for i in pending_idxs:
        ...
        condition_idx = str(srow["condition_idx"])
        ...
        scen = lookup_scenario_row(scenario_idx, scenario_df)
        ...
        print(f"[Runner] [{len(done_ids)+processed}/{total}] {condition_idx} -> {row_out['status']} ({pct:.1f}%)")
...
if __name__ == "__main__":
    ...
    run_with_schedule(
        ...
        scenario_csv=os.path.join(root, cfg.scenario_csv),
